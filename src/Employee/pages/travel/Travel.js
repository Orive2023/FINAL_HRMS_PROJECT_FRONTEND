...
const Travel = () => {
  ...
  const convertToPdf = () => {
    try {
      ...
      doc.autoTable({
        head: [["SL", "EMPLOYEE NAME", "START DATE", "END DATE", "PLACE OF VISIT"]],
        body: travel.map((row, index) => [
          index + 1,
          row.employeeName,
          row.startDate,
          row.endDate,
  ...
  const loadtravel = async () => {
    const result = await axios.get(
      `http://localhost:8082/travels/findtravels/${username}`
    );
                      ...
                      : (rowsPerPage > 0
                          ...
                          .map((travel, index) => (
                            <tr key={index}>
                              ...
                              <td className="mx-2">
                                <Link
                                  to={`/employee/travel-profile/${travel.travelId}`}
                                >
