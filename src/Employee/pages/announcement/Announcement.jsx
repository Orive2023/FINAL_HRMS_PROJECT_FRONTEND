...
const Announcement = () => {
  ...
  const emptyRows =
    page > 0 ? Math.max(0, (1 + page) * rowsPerPage - announcement.length) : 0;
  ...
  const convertToPdf = () => {
    try {
      ...
      doc.autoTable({
        ...
        body: announcement.map((row, index) => [
          index + 1,
          row.companyName,
          row.title,
          row.description,
  ...
  const convertToExcel = () => {
    const ws = XLSX.utils.json_to_sheet(announcement);
                      ...
                      : (rowsPerPage > 0
                          ...
                          .map((announcements, index) => (
                            <tr key={announcements.announcementsId}>
                              ...
                              <td className="mx-2">
                                <Link
                                  to={`/employee/announcement-profile/${announcements.announcementsId}`}
                                >
