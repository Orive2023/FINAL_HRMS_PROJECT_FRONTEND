...
const ContraVoucherTable = ({
  ...
  const convertToPdf = () => {
    try {
      ...
      doc.autoTable({
        ...
        body: contraVoucher.flatMap((row, index) => {
          return row.contraVoucherListEntities.map((subRow, subIndex) => [
            index + 1,
            row.voucherType,
            row.date,
            row.reversedAccountHead,
            subRow.debit,
            subRow.credit,
            row.remark,
          ]);
  ...
  const convertToExcel = () => {
    ...
    const flattenedContraVouchers = contraVoucher.map(contraVoucher => ({
      contraVoucherId: contraVoucher.contraVoucherId,
      voucherType: contraVoucher.voucherType,
      reversedAccountHead: contraVoucher.reversedAccountHead,
      date: contraVoucher.date,
      remark: contraVoucher.remark,
      ...contraVoucher.contraVoucherListEntities[0],
    }));
    
    const ws = XLSX.utils.json_to_sheet(flattenedContraVouchers);
            ...
            : (rowsPerPage > 0
                ...
                .map((contraVoucher, index) => (
                  <tr key={contraVoucher.contraVoucherId}>
                    ...
                    <td>{contraVoucher.voucherType}</td>
                    <td>{contraVoucher.date}</td>
                    <td>{contraVoucher.reversedAccountHead}</td>
                    <td>
                      {contraVoucher.contraVoucherListEntities.map(
                        (subVoucher, subIndex) => (
                          <tr key={subIndex}>
                    ...
                    <td className="mx-2">
                      <FaTrashAlt
                        ...
                        onClick={() =>
                          deleteContraVoucher(contraVoucher.contraVoucherId)
                        }
                      />
                    </td>
                  </tr>
