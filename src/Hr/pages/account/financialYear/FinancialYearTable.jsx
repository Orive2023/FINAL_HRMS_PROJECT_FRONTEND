...
const FinancialYearTable = ({
  ...
  const emptyRows =
    page > 0
      ? Math.max(0, (1 + page) * rowsPerPage - financialYearSection.length)
      : 0;
  ...
  const convertToPdf = () => {
    try {
      ...
      doc.autoTable({
        ...
        body: financialYearSection.map((row,index) => [
          index+1,
          row.financialYear,
    ...
    >
      ...
      <table className="table table-bordered table-hover shadow">
        ...
        <tbody className="text-center">
          {financialYearSection.length === 0
            ? renderFinancialYearData()
            : (rowsPerPage > 0
                ? financialYearSection.slice(
                    page * rowsPerPage,
                ...
                .filter((elem) => {
                  if (!search && search.length === 0) return elem;
                    ...
                    return (
                      ...
                      elem.financialYearStartDate
                      .toString()
                        .toLowerCase()
                        .includes(search.toLocaleLowerCase()) ||
                      elem.financialYearEndDate
                      .toString()
                        .toLowerCase()
                        .includes(search.toLocaleLowerCase()) 
                ...
                .map((financialYearSection, index) => (
                  <tr key={financialYearSection.expenceId}>
                    ...
                    {/* <td className="mx-2">
                      <Link
                        to={`/account/financialYearProfile/${financialYearSection.financialYearId}`}
                      >
                    ...
                    <td className="mx-2">
                      <Link
                        to={`/account/financialYearEdit/${financialYearSection.financialYearId}`}
                      >
                    ...
                    <td className="mx-2">
                      <FaTrashAlt className="action-delete"
                        onClick={() =>
                          deleteFinancial(
                            financialYearSection.financialYearId
                          )
                        }
                      />
                    </td>
