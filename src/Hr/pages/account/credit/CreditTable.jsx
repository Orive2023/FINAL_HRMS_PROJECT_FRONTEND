...
const CreditTable = ({
  ...
  const convertToExcel = () => {
    ...
    const flattenedCredit = credit.map(item => ({
      creditVoucherId: item.creditVoucherId,
      voucherType: item.voucherType,
      debitAccountHead: item.debitAccountHead,
      date: item.date,
      remark: item.remark,
      ...item.creditVoucherTableEntities[0], 
      total: item.total
    }));
  
    const ws = XLSX.utils.json_to_sheet(flattenedCredit);
          ...
          <tbody className="text-center">
            {credit.length === 0
              ...
              : (rowsPerPage > 0
                  ...
                  .map((credit, index) => (
                    <tr key={credit.creditVoucherId}>
                      <th scope="row" key={index}>
                        {index + 1}
                      </th>
                      <td>{credit.voucherType}</td>
                      <td>{credit.date}</td>
                      <td>{credit.total}</td>
                      <td>{credit.remark}</td>

                      <td className="mx-2">
                        <Link to={`/credit-profile/${credit.creditVoucherId}`}>
                          <FaEye className="action-eye" />
                        </Link>
                      </td>
                    { /* <td className="mx-2">
                        <Link to={`/edit-credit/${credit.creditVoucherId}`}>
                      ...
                      <td className="mx-2">
                        <FaTrashAlt
                          ...
                          onClick={() => deleteCredit(credit.creditVoucherId)}
                        />
                      </td>
                    </tr>
                  ))}
