...
const BankTable = ({
  ...
  const convertToPdf = () => {
    try {
      ...
      doc.autoTable({
        head: [
          ["SL", "BANK NAME", "ACCOUNT NAME", "ACCOUNT NUMBER", "BRANCH NAME"],
        ],
        body: addbank.map((row, index) => [
          index + 1,
          row.bankName,
          row.accountName,
          row.accountNumber,
              ...
              : (rowsPerPage > 0
                  ...
                  .map((addbank, index) => (
                    <tr key={addbank.id}>
                      ...
                      <td>
                      <span className={addbank.status === "Active" ? "complete" : "incomplete"}>

                      {addbank.status?addbank.status:<p className="nil">--nil--</p>}
                    </span>
                      </td>
                      <td className="mx-2">
                        <Link to={`/bank/add-bank-profile/${addbank.addBankId}`}>
                          <FaEye className="action-eye" />
                        </Link>
                      </td> 
                      <td className="mx-2">
                        <Link to={`/bank/edit-bank/${addbank.addBankId}`}>
