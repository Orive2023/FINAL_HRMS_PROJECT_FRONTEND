...
const PerformanceIndicatorView = () => {
  ...
  const savePerformance = async (e) => {
    e.preventDefault();
    await axios.post(
      "http://13.126.190.50:5000/performance/create/performance",
      formData
    );
  ...
  const loadPerformance = async () => {
    const result = await axios.get(
      "http://13.126.190.50:5000/performance/get/performance",
      {
        validateStatus: () => {
          return true;
        },
  ...
  const handleDelete = async (id) => {
    console.log(id);
    await axios.delete(`http://13.126.190.50:5000/performance/delete/${id}`);
    loadPerformance();
  };
