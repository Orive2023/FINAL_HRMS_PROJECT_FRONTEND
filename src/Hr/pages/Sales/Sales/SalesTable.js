  ...
  const convertToPdf = () => {
    try {
      ...
      doc.autoTable({
        ...
        body: sales.map((row, index) => [
          index+1,
          row.developerCost,
          row.perUserPrice,
          row.totalNumberOfUser,
          row.totalCost,
          row.status,
    ...
    >
      ...
      <div className="table-start-container">
        <table id="table" className="table table-bordered table-hover shadow">
          ...
          {sales.length === 0 ? renderSalesData() : (rowsPerPage > 0
          ...
          .filter((elem)=>{
            if(search.length===0)
              ...
              return (elem.developerCost.toString().toLowerCase().includes(search.toLocaleLowerCase()) ||
              elem.perUserPrice.toString().toLowerCase().includes(search.toLocaleLowerCase()) ||
              elem.perUserPrice.toString().toLowerCase().includes(search.toLocaleLowerCase()) ||
              elem.totalNumberOfUser.toString().toString().toLowerCase().includes(search.toLocaleLowerCase()) ||
              elem.totalCost.toString().toString().toLowerCase().includes(search.toLocaleLowerCase()) ||
              elem.status.toString().toString().toLowerCase().includes(search.toLocaleLowerCase()) 
            
              )
          }).map((sales, index) => (
                  <tr key={index}>
                    <th scope="row">{index + 1}</th>
                    <td>{sales.developerCost}</td>
                    <td>{sales.perUserPrice}</td>
                    <td>{sales.totalNumberOfUser}</td>
                    <td>{sales.totalCost}</td>
                    <td>{sales.status}</td>
                    <td className="mx-2">
                    <Link to={`/sales/sales-view/${sales.saleId}`}>
                      <FaEye className="action-eye" />
                    </Link>
                  </td> 
                     <td className="mx-2">
                    <Link to={`/sales/edit-sales/${sales.saleId}`}>
                    ...
                    <td className="mx-3">
                      <FaTrashAlt
                        ...
                        onClick={() => deleteSales(sales.saleId)}
                      />
                    </td>
                  </tr>
                ))}
