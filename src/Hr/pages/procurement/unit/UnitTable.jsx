...
const UnitTable = ({ unit, setRecDelete, setOpen, toggle }) => {
  ...
  const createPdf = () => {
    try {
      ...
      doc.autoTable({
        head: [["SL", "UNIT NAME"]],
        body: unit.map((row, index) => [index + 1, row.unitName]),
        styles: { fontSize: 5, fontStyle: "normal" },
        headStyles: {
          fillColor: [206, 206, 206],
          textColor: [20, 25, 40],
            ...
            : (rowsPerPage > 0
                ...
                .map((unit, index) => (
                  <tr key={unit.unitsId}>
                    ...
                    <td>{unit.unitName}</td>
                    <td className="mx-2">
                      <Link to={`/unit-profile/${unit.unitsId}`}>
                        <FaEye className="action-eye" />
                      </Link>
                    </td>

                    <td className="mx-2">
                      <div onClick={() => deleteUnit(unit.unitsId)}>
